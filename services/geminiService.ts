...
import { ProcessedForecasts, ForecastView, Location, ProcessedHourlyData } from '../types';
...
function formatDailyDataForPrompt(dailyData: ProcessedHourlyData[]): string {
    ...
    dataToShow.forEach((p, i) => {
        if (p.temperature_2m_max !== null && p.temperature_2m_max > maxTemp) {
            maxTemp = p.temperature_2m_max;
            warmestDayIndex = i;
        }
        if (p.temperature_2m_min !== null && p.temperature_2m_min < minTemp) {
